...
headers3 <- c('EXPOCODE', 'Cruise_ID', 'Station_ID', 'Patch', 'Cast_number', 'Event_number', 'Rosette_position', 'Niskin_flag', 'Sample_ID', 'Datetime_UTC', 'Latitude', 'Longitude', 'CTDPRES', 'CTDTEMP_ITS90', 'CTDTEMP_flag', 'Depth', 'CTDSAL_PSS78', 'CTDSAL_flag', 'CTDOXY', 'CTDOXY_flag', 'pH_NBS_measured_electrode', 'pH_flag', 'Chl_a_sensor', 'Chl_a_flag', 'Rhodamine', 'Rhodamine_flag', 'DIC', 'DIC_flag', 'TA', 'TA_flag', 'Silicate', 'Silicate_flag', 'Phosphate', 'Phosphate_flag', 'Nitrate_and_Nitrite', 'Nitrate_and_Nitrite_flag', 'Ammonium', 'Ammonium_flag', 'Particulate_Inorganic_Carbon', 'PIC_flag', 'Particulate_Organic_Carbon', 'POC_flag', 'Total_Particulate_Nitrogen', 'TPN_flag', 'Delta_C13_DIC', 'Delta_C13_DIC_flag', 'Delta_C13_POC', 'Delta_C13_POC_flag', 'Delta_N15_TPN', 'Delta_N15_TPN_flag')

ctd_bott_recal <- ctd_bott_recal[, headers3]

unit <- c("NA", "NA", "NA", "NA", "NA", "NA","NA", "NA","NA","[yyyy-mm-dd hh:mm:ss]", "decimalDegree", "decimalDegree", "decibar", "degreeCelsius", "NA", "meter", "PSS78", "NA", "micromolePerKilogram", "NA","NA","NA", "microgramPerKilogram", "NA", "microgramPerKilogram", "NA", "micromolePerKilogram", "NA", "micromolePerKilogram", "NA", "micromolePerKilogram", "NA", "micromolePerKilogram", "NA", "micromolePerKilogram", "NA", "micromolePerKilogram", "NA", "micromolePerKilogram", "NA", "micromolePerKilogram", "NA", "micromolePerKilogram", "NA", "perMil", "NA", "perMil", "NA", "perMil", "NA")
...
# paper authors: Adam V. Subhas, Jennie E. Rheuban, Zhaohui Aleck Wang, Daniel C. McCorkle, Anna P. M. Michel, Lukas Marx, Chloe L. Dean, Kate Morkeski, Matthew G. Hayden, Mary Burkitt-Gray, Francis Elder, Yiming Guo, Heather H. Kim, Ke Chen
# authors last first, initials: Subhas, A. V, Rheuban, J. E., Wang, Z. A., McCorkle, D. C., Michel, A. P. M., Marx, L., Dean, C. L., Morkeski, K., Hayden, M. G., Burkitt-Gray, M., Elder, F., Guo, Y., Kim, H. H., Chen, K.

writeLines(c("# Expocode: 33LQ20230901", "# Vessel name: Connecticut", "# PIs: A. V. Subhas; J. E. Rheuban; Z. A. Wang; D. C. McCorkle; A. P. M. Michel; L. Marx; C. L. Dean; K. Morkeski; M. G. Hayden; M. Burkitt-Gray; F. Elder; Y. Guo; H. H. Kim; K. Chen", "# Org: Woods Hole Oceanographic Institution", "# Vessel type: Ship", "# Suggested QC: B", "EXPOCODE, Cruise_ID, Datetime_UTC, Latitude, Longitude, Depth, Temperature_lab_ITS90, Temperature_lab_flag, Temperature_hull_ITS90, Temperature_hull_flag, Salinity_PSS78, Salinity_flag, Rhodamine, Rhodamine_flag, TA, TA_flag, xCO2_EQU, xCO2_ATM, xCO2_ATM_interpolated, Pressure_EQU, Pressure_ATM, Temperature_EQU_ITS90, fCO2_SW_SST, fCO2_ATM_interpolated, dfCO2, CO2_flag, CO2_subflag","NA, NA, [yyyy-mm-dd hh:mm:ss], decimalDegree, decimalDegree, meter, degreeCelsius, NA, degreeCelsius, NA, PSS78, NA, microgramPerKilogram, NA, micromolePerKilogram, NA, microatmosphere, microatmosphere, microatmosphere, hectopascal, hectopascal, degreeCelsius, microatmosphere, microatmosphere, microatmosphere, NA, NA"), here('output','LOC-01_underway_with_meta.txt'))
...
```
# read csv files and output with header lines and units
...
# temporal
startdate <- as.Date(min(underway$Datetime_UTC, na.rm = TRUE))
print(paste("Start date is", startdate))
enddate <- as.Date(max(underway$Datetime_UTC, na.rm = TRUE))
print(paste("End date is", enddate))

# Geographic coverage
North <- max(underway$Latitude, na.rm = TRUE)
East <- max(underway$Longitude, na.rm = TRUE)
South <- min(underway$Latitude, na.rm = TRUE)
West <- min(underway$Longitude, na.rm = TRUE)

print(paste("North Latitude extent is", North))
print(paste("West Latitude extent is", West))
print(paste("East Latitude extent is", East))
print(paste("South Latitude extent is", South))
