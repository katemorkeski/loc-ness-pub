...
# TODO: delete drifter1 rows after 231 (all NA)
# TODO: convert datetime to ISO
# TODO: rename Lattitude
...
drifter3 <- drifter3 |>
  ...
  relocate(Datetime_UTC_tracker, .after = Asset) |>
  relocate(Deployment, .after = Datetime_UTC_tracker) |>
  rename(Latitude = Lattitude) |>
  relocate(Latitude, .after = Deployment) |>
...
ggplot(drifter3, aes(x = Datetime_UTC_sensor, y = temp_diff)) +
  geom_line() +
  geom_point() +
  theme_minimal()
ggsave('drifter3_flTemp-AqTemp.png')

ggplot(drifter3, aes(x = Datetime_UTC_sensor, y = temp_diff)) +
  geom_line() + ylim(NA, 1) +
  geom_point() +
  theme_minimal()
ggsave('drifter3_flTemp-AqTemp_max1.png')
...
#TODO convert mg to micromol #drifter3$Oxygen_mg_kg <- drifter3$RDOConcentration.mgL/drifter3$density 
drifter3$Oxygen <- round(drifter3$Oxygen, 2)

# organize sensor columns
drifter3 <- drifter3 |>
  rename(Datetime_UTC_sensor = datetime_UTC) |>
  relocate(Datetime_UTC_sensor, .after = Speed.mph) |> 
...
# Apply OCADS-style column names
drifter3 <- drifter3 |>
...
drifter4 <- drifter4 |>
  select(-X, -Time, -Bat.Volts, -Sensor.Ppb, -Gain) 
